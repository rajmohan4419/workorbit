...
function PdfMerge() {
  ...
  const merge=async()=>{if(files.length<2){setStatus('Choose at least two PDFs.');return;}setBusy(true);setStatus('Merging PDFs locally…');try{const out=await PDFDocument.create();for(const file of files){const src=await PDFDocument.load(await file.arrayBuffer());const pages=await out.copyPages(src,src.getPageIndices());pages.forEach(p=>out.addPage(p));}const bytes=await out.save();downloadBlob(new Blob([bytes],{type:'application/pdf'}),'orbitboard-merged.pdf');setStatus('Done — merged PDF downloaded.');}catch(e){setStatus('Could not merge these PDFs. Some encrypted or malformed PDFs may not be supported.');}finally{setBusy(false);}};
...
function PdfExtract() {
  ...
  const extract=async()=>{if(!file){setStatus('Choose a PDF first.');return;}setBusy(true);try{const src=await PDFDocument.load(await file.arrayBuffer());const max=src.getPageCount();const indexes=pages.split(',').flatMap(part=>{const [a,b]=part.trim().split('-').map(Number);if(Number.isFinite(a)&&Number.isFinite(b)){const s=Math.min(a,b),e=Math.max(a,b);return Array.from({length:e-s+1},(_,i)=>s+i-1);}if(Number.isFinite(a))return [a-1];return [];}).filter(i=>i>=0&&i<max);if(!indexes.length){setStatus('Enter page numbers such as 1,3,5-7.');return;}const out=await PDFDocument.create();const copied=await out.copyPages(src,indexes);copied.forEach(p=>out.addPage(p));const bytes=await out.save();downloadBlob(new Blob([bytes],{type:'application/pdf'}),'orbitboard-extracted-pages.pdf');setStatus('Done — extracted PDF downloaded.');}catch{setStatus('Could not extract pages from this PDF.');}finally{setBusy(false);}};
...
function PdfCompressor() {
  ...
  const compress=async()=>{
    ...
    try{const src=await PDFDocument.load(await file.arrayBuffer());const bytes=await src.save({useObjectStreams:true,addDefaultPage:false});const blob=new Blob([bytes],{type:'application/pdf'});downloadBlob(blob,(file.name.replace(/\\.pdf$/i,'')||'orbitboard')+'-optimized.pdf');setStats({before:file.size,after:blob.size});setStatus('Done — optimized PDF downloaded.');}catch{setStatus('Could not optimize this PDF. Some encrypted or malformed PDFs may not be supported.');}finally{setBusy(false);}
  };
...
function PdfWorkspace() {
  ...
  const exportPdf=async()=>{if(!file||!pages.length){setStatus('No pages available.');return;}setBusy(true);try{const src=await PDFDocument.load(await file.arrayBuffer()),out=await PDFDocument.create();const nums=pages.map(p=>p.number-1).filter(i=>i>=0&&i<src.getPageCount());const copied=await out.copyPages(src,nums);copied.forEach((p,i)=>{p.setRotation({angle:pages[i].rotation});out.addPage(p);});const bytes=await out.save();downloadBlob(new Blob([bytes],{type:'application/pdf'}),'orbitboard-edited.pdf');setStatus('Done — edited PDF downloaded.');}catch{setStatus('Could not export this PDF.');}finally{setBusy(false);}};
