...
import { Activity, ArrowLeft, CalendarDays, FlaskConical, Newspaper, Play } from 'lucide-react';
      ...
      <main className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 py-8">
        ...
        <section className="mt-6 rounded-3xl border border-slate-800 bg-gradient-to-br from-violet-950/50 via-slate-900 to-slate-950 p-6 sm:p-8">
          ...
          <div className="mt-5 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            ...
            <div className="grid grid-cols-2 gap-3 min-w-[260px]"><Stat label="Latest demo close" value={formatNumber(latest.close)} /><Stat label="Demo period change" value={`${periodChange >= 0 ? '+' : ''}${periodChange.toFixed(2)}%`} /></div>
          </div>
        </section>

        <nav className="mt-5 flex gap-2 overflow-x-auto border-b border-slate-800 pb-px">
          {[['overview','Overview'],['market','Market'],['news','News'],['events','Events'],['experiments','Experiments']].map(([key,label]) => (
        ...
        {tab === 'overview' && <Overview latest={latest} experiment={experiment} onExperiments={() => setTab('experiments')} />}
        ...
        {tab === 'experiments' && <Experiments experiment={experiment} onBuild={() => navigate('/market/INFY/experiment')} />}

        <div className="mt-8 rounded-2xl border border-amber-900/50 bg-amber-950/20 p-5 text-sm leading-6 text-amber-200/80"><strong className="text-amber-200">Prototype boundary:</strong> every market observation, event and news item on this page is synthetic. This build has no live prices, broker connection, order execution or investment recommendation.</div>
      </main>
    </div>
  );
}

function Overview({ latest, experiment, onExperiments }) {
  const five = experiment?.results?.[5];
  return <section className="mt-6 grid lg:grid-cols-[1.1fr_.9fr] gap-6">
    <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
      <p className="text-xs font-bold uppercase tracking-[.18em] text-violet-400">Research snapshot</p><h2 className="mt-2 text-2xl font-black">What is in the record?</h2>
      <div className="mt-6 grid sm:grid-cols-3 gap-3"><Stat label="Latest close" value={formatNumber(latest.close)} icon={Activity} /><Stat label="Data sessions" value="520" icon={CalendarDays} /><Stat label="Synthetic events" value="11" icon={Newspaper} /></div>
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
      <p className="text-xs font-bold uppercase tracking-[.18em] text-emerald-400">Experiment evidence</p><h2 className="mt-2 text-2xl font-black">One reproducible test</h2>
      {five && <div className="mt-5 grid grid-cols-2 gap-3"><Stat label="Observations" value={five.observations} /><Stat label="Positive rate" value={five.positiveRate == null ? '—' : `${five.positiveRate.toFixed(1)}%`} /><Stat label="Median return" value={five.medianReturn == null ? '—' : `${five.medianReturn >= 0 ? '+' : ''}${five.medianReturn.toFixed(2)}%`} /><Stat label="Benchmark" value="Not supplied" /></div>}
...
function Market({ rows }) {
  ...
  return <section className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/70 p-6"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-cyan-400">Market</p><h2 className="mt-1 text-2xl font-black">Historical session explorer</h2></div><span className="text-xs text-slate-500">{row.date}</span></div><div className="mt-6 grid grid-cols-2 md:grid-cols-5 gap-3">{[['Open',row.open],['High',row.high],['Low',row.low],['Close',row.close],['Volume',row.volume]].map(([label,value]) => <Stat key={label} label={label} value={formatNumber(value)} />)}</div><input aria-label="Historical market session" type="range" min="0" max={rows.length - 1} value={index} onChange={(e) => setIndex(Number(e.target.value))} className="mt-8 w-full accent-cyan-500" /><p className="mt-3 text-xs text-slate-600">Replay is based on the synthetic dataset and validates the research workflow.</p></section>;
}

function News({ items }) {
  return <section className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/70 p-6"><p className="text-xs font-bold uppercase tracking-[.18em] text-blue-400">News</p><h2 className="mt-1 text-2xl font-black">Event-linked news</h2><div className="mt-6 space-y-3">{items.map(item => <article key={item.id} className="rounded-xl border border-slate-800 bg-slate-950 p-4"><div className="flex justify-between gap-4"><h3 className="font-bold">{item.headline}</h3><time className="shrink-0 text-xs text-slate-600">{item.publishedAt.slice(0,10)}</time></div><p className="mt-2 text-sm text-slate-500">{item.summary}</p><span className="mt-3 inline-block text-[11px] font-semibold text-slate-600">{item.publisher}</span></article>)}</div></section>;
}

function Events({ items }) {
  return <section className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/70 p-6"><p className="text-xs font-bold uppercase tracking-[.18em] text-amber-400">Company events</p><h2 className="mt-1 text-2xl font-black">Timeline</h2><div className="mt-6 space-y-3">{items.map(item => <article key={item.id} className="flex gap-4 rounded-xl border border-slate-800 bg-slate-950 p-4"><CalendarDays size={18} className="mt-0.5 shrink-0 text-amber-400" /><div><h3 className="font-bold">{item.title}</h3><p className="mt-1 text-sm text-slate-500">{item.description}</p><time className="mt-2 block text-xs text-slate-600">{item.timestamp}</time></div></article>)}</div></section>;
}

function Experiments({ experiment, onBuild }) {
  ...
  return <section className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-violet-400">Experiment engine</p><h2 className="mt-1 text-2xl font-black">Volume + OI hypothesis</h2></div><button type="button" onClick={onBuild} className="inline-flex items-center gap-2 rounded-xl bg-violet-500 px-4 py-2 text-sm font-bold"><Play size={15} /> Build an experiment</button></div>
